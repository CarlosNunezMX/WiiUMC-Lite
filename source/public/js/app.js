...
function onDirectorys(directorys){
    var inner = ""
    ...
    directorys.forEach(function (e, i) {
        ...
        inner += '</div><div class="col-9"> ' + e.name + '</div>' 
        inner += '</div>'
    })  
...
files.addEventListener('click', function(event){
    var id = event.path.find(function(e){ return e.id }).id;
    ...
    if(i.type === 'video'  || i.type === "addon_video"){
        var inner = '<video src="' + i.path + '" controls autoplay/>'
        vid.innerHTML = inner
    }

    if(i.type === "addon_folder"){
        console.log(i)
